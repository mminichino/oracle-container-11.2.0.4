...
function err_exit {
    if [ -z "$1" ]; then
       echo "Usage: $0 -s ORACLE_SID -d /db/mount [ -m | -p password | -v version | -n name | -P port | -S shm_size ]"
    else
       echo "$1"
    fi
    exit 1
...
while getopts "ms:d:p:n:v:P:S:" opt
do
  case $opt in
    m)
      OPTS="-it"
...
if [ -f $DATA_MOUNT/dbconfig/*.dbconfig ]; then
   . $DATA_MOUNT/dbconfig/*.dbconfig
   if [ -n "$DBVERSION" -a "$VERSION_OPT" -eq 0 ]; then
      echo "Found DB $DBVERSION configuration on $DATA_MOUNT ..."
